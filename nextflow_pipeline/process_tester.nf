...
process make_scratch_directory {
    ...
    output:
        tuple "${scratch_sequence}/${run_directory}", fastq_filepath.baseName, val(organism), val(strandedness), val(run_directory) into scratch_run_directory_ch
...
process rsync_from_lts {
    ...
    input:
    set fastq_scratchpath, fastq_basename, organism, strandedness, run_directory from scratch_run_directory_ch

    script:
    """
    echo "${fastq_scratchpath} ${fastq_basename} ${organism} ${strandedness} ${run_directory}" >> /scratch/mblab/chasem/nextflow_output_tester.txt
    """
