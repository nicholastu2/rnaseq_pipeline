...
class StandardData:
    ...
    def __init__(self, expected_attributes=None, *args, **kwargs):
        ...
        if not kwargs['config_file']:
            if not os.path.exists('/opt/apps/labs/mblab/software/rnaseq_pipeline/1.0/config/rnaseq_pipeline_config.ini'):
                raise('default config path not found. enter a config path in the StandardObject or StandardObject child call')
        ...
        # this will create a logger with default settings without a config file -- see utils
    ...
    def standardDirectoryStructure(self):
        ...
        if not self.interactive:
            ...
            utils.softLinkAndSetAttr(self, lts_dirs_to_softlink, self.lts_rnaseq_data, self.user_rnaseq_pipeline_directory)
        ...
        process_directories = ['reports', 'query', 'sbatch_log', 'log/%s' % self.year_month_day, 'job_scripts', 'rnaseq_tmp']  # TODO: MAKE SBATCH_LOG LIKE LOG WITH YEAR_MONTH_DAY SUBDIR
