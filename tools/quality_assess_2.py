...
def main(argv):
    ...
    od = OrganismData(organism=parsed.organism, query_sheet_path=parsed.query_sheet_path,
                      ...
                      experiment_conditions=parsed.experimental_conditions)
    ...
    if os.path.exists(output_name):
        print(
            'WARNING: %s already exists. If you continue, the file will be overwritten. Do you wish to continue (y/n)?\n' % output_name)
        user_response = input()
        if user_response == 'n':
            sys.exit('goodbye')
    if not os.path.exists(od.norm_count_matrix):
    ...
    if od.drug_markers is None:
        drug_markers = None
        drug_marker_columns = []
    else:
        drug_markers = od.drug_markers
        drug_marker_columns = [drug_marker + '_FOM' for drug_marker in od.drug_markers]
    ...
    df, rep_max = initialize_dataframe(parsed.query_sheet, df_columns, od.experiment_conditions)
    ...
    if rep_max > 7:
        ...
        if user_response == 'n':
            ...
            expr, sample_dict = load_expression_data(df, parsed.count_matrix, parsed.gene_list, od.experiment_conditions)
            ...
            if parsed.descriptors_specific_fow:
                df = assess_efficient_mutation(df, expr, sample_dict, parsed.wildtype, od.experiment_conditions)
            else:
    ...
    df = assess_replicate_concordance(df, expr, sample_dict, od.experiment_conditions)
    print('... Auto auditing')
    df = update_auto_audit(df, parsed.auto_audit_threshold)
    save_dataframe(output_name, df, df_columns, od.experiment_conditions, len(od.experiment_conditions))
...
def parse_args(argv):
    ...
    parser.add_argument('-d', '--drug_marker', default=None,
                        help='drug markers inserted to replace the deleted genes. Use "," as delimiter '
                             'if multiple cassettes exist.')
    ...
    parser.add_argument('--qc_config', default='/opt/apps/labs/mblab/software/rnaseq_pipeline/1.0/templates/qc_config.yaml',
...
def initialize_dataframe(query, df_cols, conditions):
    ...
    df2 = df2[['GENOTYPE', 'REPLICATE', 'COUNTFILENAME'] + conditions]
...
def load_expression_data(df, cnt_mtx, gene_list, conditions):
    ...
    if gene_list:
        ...
        if len(np.setdiff1d(gids, count['gene'])) > 0:
            print(
                'WARNING: The custom gene list contains genes that are not in count matrix. Proceeding using the intersection.')
