...
def make_snapshot_filename(region, height, suffix=None, fig_format='png'):
    ...
    if len(region) >= 4:
        ...
        return (
            '{}'.format(name))  # '{}.png'.format(name) # don't include file extension! user must do this ahead of time!
    elif len(region) == 3:
...
def write_IGV_script(input_files, region_file, IGV_batchscript_file, IGV_snapshot_dir, genome_version, image_height,
                     suffix=None, nf4_mode=False, fig_format='png'):
    """
...
def run_IGV_script(igv_script, igv_jar, memMB):
    ...
    igv_command = "xvfb-run --auto-servernum --server-num=1 java -Xmx{}m -jar {} -b {}".format(memMB, igv_jar,
                                                                                               igv_script)
    print('\nIGV command is:\n{}\n'.format(igv_command))
...
def main(input_files, region_file='regions.bed', genome='hg19', image_height='500', outdir='IGV_Snapshots',
         igv_jar_bin="bin/IGV_2.3.81/igv.jar", igv_mem="4000", no_snap=False, suffix=None, nf4_mode=False,
         onlysnap=False, fig_format='png'):
    """
    ...
    write_IGV_script(input_files=input_files, region_file=region_file, IGV_batchscript_file=batchscript_file,
                     IGV_snapshot_dir=outdir, genome_version=genome, image_height=image_height, suffix=suffix,
...
def run():
    ...
    parser.add_argument("input_files", nargs='+',
                        help="pathes to the files to create snapshots from e.g. .bam, .bigwig, etc.")  # , nargs='?'

    # required flags
    parser.add_argument("-r", default='regions.bed', type=str, dest='region_file', metavar='regions',
                        help="BED file with regions to create snapshots over")

    # optional flags
    parser.add_argument("-g", default='hg19', type=str, dest='genome', metavar='genome',
                        help="Name of the reference genome, Defaults to hg19")
    ...
    main(input_files=input_files, region_file=region_file, genome=genome, image_height=image_height, outdir=outdir,
         igv_jar_bin=igv_jar_bin, igv_mem=igv_mem, no_snap=no_snap, suffix=suffix, nf4_mode=nf4_mode, onlysnap=onlysnap,
