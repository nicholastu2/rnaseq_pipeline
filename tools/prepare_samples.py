...
def populate_sample_summary(df, metadata, df_cols, qc_cols, conditions, group=None):
    ...
    for i, row in df2.iterrows():
        ...
        if sample_descriptor in exist_samples:
            ...
            print('WARNING: found existing sample: %s' % ' '.join(
                [':'.join([col, str(row[col])]) for col in sorted(check_cols)]))
            continue
        ...
        if len(files_found) == 0:
            ...
            sys.exit('ERROR: Mutilple files exist with the same index %s and run number %s.' % (
            row['INDEX'], row['RUN_NUMBER']))
        else:
...
def main(argv):
    df_meta_columns = ['GENOTYPE', 'STRAIN', 'SAMPLE', 'INDUCTION', 'LIBRARY', 'REPLICATE', 'INDEX', 'RUN_NUMBER',
                       'FILE', 'GROUP']
    df_qc_columns = ['ST_PIPE', 'ST_TOTAL_READS', 'ST_ALIGN_PCT', 'ST_MUT_FOW', 'ST_RC_FOM', 'ST_COV_MED', 'AUTO_AUDIT',
